<template>
    <n-card :class="`shadow-lg`" :segmented="{
        ...
        <template #header>Tambah Pembayaran Sebagian Remidial</template>
        <span class="hidden">{{ pelunasan }}</span>
        <template #header-extra>

        </template>
        <div>
            <n-form-item label="Cabang Pembayaran">
                <n-select :loading="loadingBranch" filterable placeholder="Pilih Cabang" label-field="nama"
                    value-field="id" :default-value="defBranch" :options="dataBranch" v-model:value="selectBranch" />
            </n-form-item>
        </div>
        <div class="flex flex-col md:flex-row gap-2" v-show="!props.embed" v-if="selectBranch">
    ...
    :on-update:checked-row-keys="handleFasilitas" :loading="loadSearch" class="pb-2" v-show="dataFasilitas" /> -->
        ...
        <n-spin v-if="displayDetail" :show="spinnerShow">
            ...
            <div>
                ...
                <n-alert title="info" type="warning" v-if="pelunasan.DISKON_POKOK !== 0">
...
const postDynamic = async () => {
    ...
    pelunasan.CABANG = selectBranch.value;
