<template>
    <div class="flex p-1 border bg-slate-50 shadow rounded-lg">
        <div class="flex flex-col p-2 w-1/4 justify-between">
            <div class="flex flex-col sticky top-4">
                ...
                <div class="py-4 flex flex-col gap-y-2">
                    <n-button @click="addParam(param)" type="primary" quaternary :key="param.id"
                        v-for="param in paramScoring"
                        class="w-full justify-start p-2  flex  cursor-pointer hover:bg-slate-200 rounded"
                        :disabled="false">
                        <n-icon size="large">
                            <add-icon />
                        </n-icon>
            ...
            <div class="p-4">
                <n-progress type="line" :show-indicator="false" status="info"
                    :percentage="100 / paramScoring.length * (bucketParam.length ? bucketParam.length : 0)" />
                <div class="py-2">{{ bucketParam.length ? bucketParam.length : 0 }} parameter dari {{
                    paramScoring.length }}
                </div>
        ...
        <div class="w-full bg-white min-h-[500px] max-h-[500px] rounded-lg p-4 overflow-auto">
            ...
            <div v-for="(listBucket, i) in bucketParam" :key="listBucket.key">
                <n-card :class="`shadow-lg`" :title="listBucket.param" size="small" :segmented="true">
                    ...
                    <div v-for="itemParam in listBucket.paramlist" :key="itemParam.key">
                        <n-form-item class="w-full" v-if="itemParam.mode.type == 'area'" :label="itemParam.title">
                            <n-input type="textarea" placeholder="Isi disini" v-model:value="itemParam.val" />
                        </n-form-item>
                        <n-card :class="`shadow-lg`" embedded v-else-if="itemParam.mode.type == 'table'"
                            :title="itemParam.title" class="mb-4">
                            <template #header-extra>
                            ...
                            <n-data-table ref="tableRef" :data="dataParse?.data"
                                :columns="convertObjectToArray(dataParse?.data)" />
                        </n-card>
                        <n-form-item class="w-full" v-else :label="itemParam.title">
                            <n-select v-model:value="itemParam.val" placeholder="Silakan pilih"
    ...
    <n-modal v-model:show="prevModal">
        <n-card :class="`shadow-lg`" class="w-4/6">
            <n-watermark ref="printAreaRef" cross selectable :font-size="16" :line-height="16" :width="192"
                :height="128" :x-offset="12" :y-offset="28" :rotate="-10" class="p-4">
                <div class="flex flex-col gap-y-4">
                    ...
                    <div class="" v-for="(param, k) in _.sortBy(bucketParam, [function (o) { return o.key; }])"
                        :key="param.key">
...
<script setup>
import { AddCircleRound as AddIcon, CloseRound as RemoveIcon } from "@vicons/material";
import _ from "lodash";
import { NButton, NDatePicker, NInput, useLoadingBar } from "naive-ui";
