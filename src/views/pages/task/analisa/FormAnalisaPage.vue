<template>
    <n-card :class="`shadow-lg`" title="form isian analisa">
            ...
            <n-space vertical>
                <n-collapse>
                    <n-steps vertical v-model:current="currentRef">
                        <n-step v-for="schema in schemaScoring" :key="schema.id" :title="schema.kriteria">
                                    ...
                                    <div class="flex flex-col w-full">
                                        ...
                                        <div class="p-2 bg-white/50 rounded-lg">
                                            <div class="flex gap-4 items-center">
                                                ...
                                                <div
                                                    class="text-2xl w-[50px] justify-center font-bold border flex rounded-lg">
                                                    {{
                                                        header.nilai
                                                    }}
                                                </div>
                                            </div>
                                            <div>{{ header.selected_param = findMatchParameter(header.param,
                                                header.nilai).parameter }}</div>
                                        </div>
    ...
    <n-modal v-model:show="modalSt">
        <n-card :class="`shadow-lg`" class="w-11/12" title="Data Detail Debitur" :bordered="false" size="huge"
                ...
                <div v-for="(scoring, i) in schemaScoring" :key="scoring.id">
                    ...
                    <table class="w-full ">
                        <tr v-for="item in scoring.scoring" :key="item.id">
                            <td class="border border-black"><b>{{ item.header }}</b><br><small>{{ item.selected_param
                                    }}</small></td>
