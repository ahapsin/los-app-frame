            ...
            <n-space vertical :size="12" class="pt-4">
                <n-space>
                    <n-date-picker v-model:formatted-value="rangeDate" :default-calendar-start-time="Date.now()"
                        ...
                        format="yyyy-MM-dd" />
                    <n-select v-show="me.me.cabang_nama == 'Head Office'" :loading="loadingBranch" @click="getBranch"
                        filterable placeholder="Pilih Cabang" label-field="nama" value-field="id"
                ...
                <div class=" border p-4 border-black" v-if="tarik">
                    <div class="flex items-center gap-2 justify-between">
                        ...
                        <div class="text-md font-bold justify-center pt-4 flex flex-col text-right">
                            ...
                            <n-text class="text-sm">tanggal : {{ formatDate(dataArusKas.dari) }}-{{
                                formatDate(dataArusKas.sampai)
                                }}</n-text>
                    ...
                    <table class="w-full font-mono text-xs" cellspacing="10">
                        <thead>
                            <tr class="border-b border-t border-black border-dashed">
                                <th align="left" class="px-2" v-for="header in dataArusKas.HeaderTable">{{ header }}
                                </th>
                            </tr>
                        </thead>
